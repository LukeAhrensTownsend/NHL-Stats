...
module.exports = {
	...
	getDivisionStandings: async function(season) {
		let data = await HELPERS.asyncFetch(
			`standings/byDivision?expand=standings.record&season=${season}`
		);
		...
		data.records.map((division, index) => {
			if (index + 1 <= Math.ceil(data.records.length / 2)) {
				temp.westernConference.conferenceName =
					division.conference.name;
				temp.westernConference.divisionRecords.push({
					conferenceName: division.conference.name,
					divisionName: division.division.name,
					teamRecords: division.teamRecords,
				});
			} else {
				temp.easternConference.conferenceName =
					division.conference.name;
				temp.easternConference.divisionRecords.push({
					conferenceName: division.conference.name,
					divisionName: division.division.name,
					teamRecords: division.teamRecords,
				});
	...
	getConferenceStandings: async function(season) {
		let data = await HELPERS.asyncFetch(
			`standings/byConference?expand=standings.record&season=${season}`
		);

		return {
			easternConferenceStandings: {
				conferenceName: data.records[0].conference.name,
				teamRecords: data.records[0].teamRecords,
			},
			westernConferenceStandings: {
				conferenceName: data.records[1].conference.name,
				teamRecords: data.records[1].teamRecords,
			},
		};
	},
	getLeagueStandings: async function(season) {
		let data = await HELPERS.asyncFetch(
			`standings/byLeague?expand=standings.record&season=${season}`
		);
	...
	getTeamData: async function(teamId) {
		let data = await HELPERS.asyncFetch(
			`teams/${teamId}?expand=team.roster&expand=team.schedule.previous&expand=team.schedule.next&expand=team.stats`
		);
	...
	getSeasonGamesByTeam: async function(teamId, season) {
		let data = await HELPERS.asyncFetch(
			`schedule?teamId=${teamId}&season=${season}`
		);
